...
from .constants import EXPECTED_GAF_TAGS, EXPECTED_MAP_Q, MAP_Q_ID, READ_NAME_ID, RELATIVE_STRAND_ID, PATH_START_ID,PATH_END_ID, PATH_ID, CS_TAG_ID
...
def process_line(gaf_line: str):
    ...
    if (len(line_elements) == EXPECTED_GAF_TAGS) and map_q == EXPECTED_MAP_Q:

        #extract needed tags
        read_name = line_elements[READ_NAME_ID]
        relative_strand = True if line_elements[RELATIVE_STRAND_ID] == "+" else False
        path_start = line_elements[PATH_START_ID]
        path_end = line_elements[PATH_END_ID]
        ...
        cs_line = 0 #[ i for i in parse_cs_line(line_elements[CS_TAG_ID])]
